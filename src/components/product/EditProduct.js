...
function EditProduct() {
  ...
  const [allCategory, getCategory] = useState([]);
  ...
  const getProductById = () => {
    axios.get(`${BASE_URL}/Product/${productId}`).then((response) => {
      setProductId(response.data.productId);
      setProductName(response.data.productName);
      setCategoryId(response.data.categoryId);
      setDescription(response.data.description);
      setPrice(response.data.price);
      setProductImageName(response.data.imageUrl);
  ...
  const EditProduct = () => {
    // console.log("helo")
    axios.put(`${BASE_URL}/Product/${productId}`, objProduct,{
      headers:{
  ...
  return (
    <>
      <Helmet>
        <title>Mr.Refill | Edit Product</title>
      </Helmet>
      <Header />
      <Sidebar />
                  ...
                  <div className="card-body">
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      <div className="form-group">
                        ...
                        <select className="form-control select2-search-disable" value={categoryId} onChange={(e) => { setCategoryId(e.target.value) }}>
                      ...
                      <div className="form-group row justify-content-end">
                        <div className="col-sm-9">
                          <div>
                            <button type='button' onClick={EditProduct} className="btn btn-primary w-md mr-3" style={{ alignContent: 'center' }}>Save Changes</button>
